...
/* Pro is rendered apart from LINKS: it is the only revenue link on the site
   and was previously indistinguishable from the blog index. */
const PRO = { href: "/pro", label: "Pro" };
    ...
    >
      ...
      >
        ...
        <div className="hidden items-center gap-1 md:flex">
          ...
          {/* No container: the nav already has one solid orange button and
              Download should keep winning that contest. The looping sheen
              lives in globals.css. Hover dims rather than recolours, because
              the word's own colour is transparent — the gradient behind it is
              what you actually see. Same treatment as the wordmark above. */}
          <Link
            href={PRO.href}
            aria-current={isActive(pathname, PRO.href) ? "page" : undefined}
            title="Clipmer Pro — one-time payment"
            className={cn(
              "ml-1 inline-flex items-center rounded-md px-3 py-1.5 text-sm text-orange transition-opacity hover:opacity-80",
              isActive(pathname, PRO.href) ? "font-semibold" : "font-medium"
            )}
          >
            <span data-pro-sheen>{PRO.label}</span>
          </Link>
        ...
        >
          <div className="mx-auto max-w-7xl space-y-1 px-4 py-4 sm:px-6">
            ...
            <Link
              ...
              aria-current={isActive(pathname, PRO.href) ? "page" : undefined}
              className={cn(
                "block rounded-md px-3 py-2.5 text-[0.9375rem] text-orange",
                isActive(pathname, PRO.href) ? "font-semibold" : "font-medium"
              )}
            >
              <span data-pro-sheen>{PRO.label}</span>
